-- =============================================
-- FUNCIONES RPC QUE FALTAN - CORREGIDO
-- =============================================
...
DECLARE
  ...
  v_ranking INTEGER := 1; -- Por ahora siempre Top 3
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(EXTRACT(EPOCH FROM (end_date - start_date)) / (7 * 24 * 60 * 60))::INTEGER, 0),
...
BEGIN
  ...
  SELECT member_id INTO v_master_id
  FROM masters
