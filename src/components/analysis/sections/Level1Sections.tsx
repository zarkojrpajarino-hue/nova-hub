    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-base">
            <Gauge className="h-5 w-5 text-gray-500" />{t('analysis.resumenEjecutivo')}</CardTitle>
          {/* Momentum score */}
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* Fortalezas */}
          <div>
            <p className="text-xs font-semibold text-green-700 dark:text-green-400 uppercase tracking-wide mb-1.5">{t('analysis.fortalezas')}</p>
          ...
          {/* Riesgos */}
          <div>
            <p className="text-xs font-semibold text-red-700 dark:text-red-400 uppercase tracking-wide mb-1.5">{t('analysis.riesgosInmediatos')}</p>
            <ul className="space-y-1">
              {data.risks.map((r, i) => (
                <li key={i} className="flex items-start gap-1.5 text-sm text-gray-700 dark:text-gray-300">
...
export function PhaseFitSection({ data }: { data: NonNullable<AnalysisSection['phase_fit']> }) {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">
          <Target className="h-5 w-5 text-gray-500" />{t('analysis.fitDeFase')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="rounded-md bg-blue-50 border border-blue-200 px-3 py-2 dark:bg-blue-950/30 dark:border-blue-800">
          <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wide mb-0.5">{t('analysis.focoRecomendadoEstaSemana')}</p>
...
export function ContradictionsSection({ data }: { data: NonNullable<AnalysisSection['contradictions']> }) {
  ...
  return (
    <Card className="border-orange-200 dark:border-orange-800">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">
          <GitCompareArrows className="h-5 w-5 text-orange-600 dark:text-orange-400" />{t('analysis.contradiccionesDetectadas')}</CardTitle>
        <p className="text-xs text-orange-600 dark:text-orange-500">{t('analysis.decisionesPasadasQueChocan')}</p>
      </CardHeader>
      <CardContent className="space-y-3">
        {data.map((item, i) => (
          <div
          ...
          >
            <div className="flex items-start gap-2">
              <Badge variant="outline" className="text-[10px] shrink-0 border-orange-300 text-orange-700">{t('analysis.decisiónPasada')}</Badge>
            ...
            <div className="flex items-start gap-2">
              <Badge variant="outline" className="text-[10px] shrink-0 border-blue-300 text-blue-700">{t('analysis.hoy')}</Badge>
...
export function UrgentDecisionsSection({ data }: { data: NonNullable<AnalysisSection['urgent_decisions']> }) {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">
          <AlertTriangle className="h-5 w-5 text-amber-500" />{t('analysis.decisionesUrgentes')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {data.map((decision, i) => (
          <div
