/**
 * MeetingInsightsCard — M18.5
 *
 * Muestra los insights activos del Meeting Agent en el ProjectEnginePanel.
...
import { AlertTriangle, Info, Loader2, Video } from 'lucide-react'
...
export function MeetingInsightsCard({ projectId }: MeetingInsightsCardProps) {
  const { data: insights, isLoading } = useQuery({
    queryKey: ['meeting_insights', projectId],
    enabled:  !!projectId,
    staleTime: 60_000,
    queryFn: () => getActiveMeetingInsights(projectId!),
  })
            ...
            >
              ...
              <div className="flex items-center gap-3 pt-1">
                <span className="text-xs text-muted-foreground/60">
                  {payload.signal.data_points} insight{payload.signal.data_points !== 1 ? 's' : ''} · Meeting Agent
                </span>
                <span className="text-xs text-muted-foreground/60">
