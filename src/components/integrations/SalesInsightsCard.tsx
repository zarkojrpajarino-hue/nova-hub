/**
 * SalesInsightsCard — I15.79
 *
 * Muestra los insights activos del Sales Agent para una conexión HubSpot.
...
import { getActiveSalesInsights } from '@/services/salesAgentService'
...
export function SalesInsightsCard({ projectId }: SalesInsightsCardProps) {
  const { data: insights, isLoading } = useQuery({
    queryKey:  ['sales_insights', projectId],
    enabled:   !!projectId,
    staleTime: 60_000,
    queryFn:   () => getActiveSalesInsights(projectId!),
  })
            ...
            >
              ...
              <div className="flex items-center gap-3 pt-1">
                <span className="text-xs text-muted-foreground/60">
                  {signal.data_points} deal{signal.data_points !== 1 ? 's' : ''} · HubSpot
                </span>
                <span className="text-xs text-muted-foreground/60">
