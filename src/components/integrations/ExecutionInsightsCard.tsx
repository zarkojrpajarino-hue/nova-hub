/**
 * ExecutionInsightsCard — I15.80
 *
 * Muestra los insights activos del Execution Agent para una conexión Asana.
...
interface ExecutionInsightsCardProps {
  projectId: string | undefined
}

export function ExecutionInsightsCard({ projectId }: ExecutionInsightsCardProps) {
  const { data: insights, isLoading } = useQuery({
    queryKey: ['execution_insights', projectId],
    enabled:  !!projectId,
    staleTime: 60_000,
    queryFn:  () => getActiveExecutionInsights(projectId!),
  })
            ...
            >
              ...
              <div className="flex items-center gap-3 pt-1">
                <span className="text-xs text-muted-foreground/60">
                  {payload.signal.data_points} tarea{payload.signal.data_points !== 1 ? 's' : ''} · Asana
                </span>
                <span className="text-xs text-muted-foreground/60">
