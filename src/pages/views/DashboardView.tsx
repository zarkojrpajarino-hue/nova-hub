...
export function DashboardView({ onNewOBV }: DashboardViewProps) {
  ...
  // Counts from engineData — sticky state: only updates when new data arrives, never resets to 0
  const [stickyCounts, setStickyCounts] = useState({ obvs: 0, kpis: 0, tasks: 0 });
  useEffect(() => {
  ...
  const leadsCount = stickyCounts.obvs;
  ...
  const kpiCount = stickyCounts.kpis;
  ...
  const tasksCompletedWeekly = stickyCounts.tasks;
