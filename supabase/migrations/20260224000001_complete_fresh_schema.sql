...
-- =============================================
-- SECTION 17: ENGINE TABLES (Optimus-K System)
-- Added: 2026-02-24
-- =============================================
...
-- tasks: leader_id
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS leader_id UUID REFERENCES profiles(id) ON DELETE SET NULL;

-- projects: geographic + cluster data
ALTER TABLE projects ADD COLUMN IF NOT EXISTS country TEXT;
ALTER TABLE projects ADD COLUMN IF NOT EXISTS market_scope TEXT CHECK (market_scope IN ('local', 'national', 'international'));
ALTER TABLE projects ADD COLUMN IF NOT EXISTS cluster TEXT;

-- ---- Phase Engine ----
-- Current phase state per project (single live row per project)
CREATE TABLE IF NOT EXISTS project_phase_state (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
...
-- ---- Probability Engine ----
-- Current win probability per project
...
CREATE TABLE IF NOT EXISTS project_probability_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
...
-- ---- Viability Engine ----
-- Go/No-Go assessment per project
CREATE TABLE IF NOT EXISTS project_viability_state (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  status TEXT NOT NULL CHECK (status IN ('viable', 'at_risk', 'critical', 'no_go')),
  score NUMERIC(5,2),
  signals JSONB,
  recommendation TEXT,
  engine_version TEXT NOT NULL DEFAULT 'v1',
...
-- Current economic model per project
CREATE TABLE IF NOT EXISTS project_economic_profile (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  model_type TEXT,
...
-- Economic profile snapshots (saved when model changes)
CREATE TABLE IF NOT EXISTS project_economic_profile_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  model_type TEXT,
  avg_ticket NUMERIC(12,2),
...
-- ---- Risk Engine ----
-- Composite risk score per project
-- Formula: RiskScore = (runway*0.25) + (revenue_concentration*0.20) +
--          (execution_drop*0.20) + (validation_weakness*0.20) + (bottleneck*0.15)
CREATE TABLE IF NOT EXISTS project_risk_score (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
...
-- ---- Org Engine ----
-- Functional coverage map per project
CREATE TABLE IF NOT EXISTS project_function_coverage (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
...
-- ---- Decision Events Log ----
-- Append-only log of all engine-driven strategic events
...
-- ---- Strategic Blocks ----
-- Detected blockers that need resolution
CREATE TABLE IF NOT EXISTS strategic_blocks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
...
-- ---- Project Protocols ----
-- Active operational protocols (focus_mode, crisis_mode, reset_ritual, etc.)
...
-- ---- Strategic Cycles ----
-- Weekly / monthly / quarterly iteration tracking
CREATE TABLE IF NOT EXISTS strategic_cycles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  cycle_type TEXT NOT NULL CHECK (cycle_type IN ('weekly', 'monthly', 'quarterly')),
...
-- ---- Benchmarks ----
-- Reference benchmarks for engine comparisons
...
CREATE INDEX IF NOT EXISTS idx_project_phase_state_project    ON project_phase_state(project_id);
CREATE INDEX IF NOT EXISTS idx_project_probability_project     ON project_probability(project_id);
CREATE INDEX IF NOT EXISTS idx_prob_history_project_time       ON project_probability_history(project_id, computed_at DESC);
CREATE INDEX IF NOT EXISTS idx_viability_state_project         ON project_viability_state(project_id);
CREATE INDEX IF NOT EXISTS idx_economic_profile_project        ON project_economic_profile(project_id);
CREATE INDEX IF NOT EXISTS idx_econ_history_project_time       ON project_economic_profile_history(project_id, computed_at DESC);
CREATE INDEX IF NOT EXISTS idx_risk_score_project              ON project_risk_score(project_id);
CREATE INDEX IF NOT EXISTS idx_function_coverage_project       ON project_function_coverage(project_id);
CREATE INDEX IF NOT EXISTS idx_decision_events_project_time    ON decision_events(project_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_decision_events_type            ON decision_events(event_type);
CREATE INDEX IF NOT EXISTS idx_strategic_blocks_project        ON strategic_blocks(project_id);
CREATE INDEX IF NOT EXISTS idx_strategic_blocks_open           ON strategic_blocks(project_id, resolved_at) WHERE resolved_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_project_protocols_project       ON project_protocols(project_id);
CREATE INDEX IF NOT EXISTS idx_project_protocols_active        ON project_protocols(project_id, deactivated_at) WHERE deactivated_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_strategic_cycles_project        ON strategic_cycles(project_id);
CREATE INDEX IF NOT EXISTS idx_benchmarks_category_segment     ON benchmarks(category, segment);

-- =============================================
-- RLS for Engine Tables
-- =============================================
...
-- Engine tables: authenticated users access their own project data.
-- Edge functions use service_role which bypasses RLS for writes.

CREATE POLICY "phase_state: owner access" ON project_phase_state
  FOR ALL TO authenticated
...
CREATE POLICY "probability: owner access" ON project_probability
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "probability_history: owner access" ON project_probability_history
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "viability_state: owner access" ON project_viability_state
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "economic_profile: owner access" ON project_economic_profile
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "economic_profile_history: owner access" ON project_economic_profile_history
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "risk_score: owner access" ON project_risk_score
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "function_coverage: owner access" ON project_function_coverage
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "decision_events: owner access" ON decision_events
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "strategic_blocks: owner access" ON strategic_blocks
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "project_protocols: owner access" ON project_protocols
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "strategic_cycles: owner access" ON strategic_cycles
  FOR ALL TO authenticated
  USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()))
  WITH CHECK (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid()));

CREATE POLICY "benchmarks: read for all authenticated" ON benchmarks
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "benchmarks: admin write" ON benchmarks
  FOR ALL TO authenticated
  USING (has_role(get_profile_id(auth.uid()), 'admin'))
  WITH CHECK (has_role(get_profile_id(auth.uid()), 'admin'));
