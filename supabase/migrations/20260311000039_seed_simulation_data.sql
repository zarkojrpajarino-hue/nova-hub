-- ============================================================================
-- MIGRACIÓN 00039 — Bootstrapping: seed_simulation_data(p_owner_id UUID)
--
-- Propósito: generar 20 proyectos simulados (5 familias × 4 variaciones) con
-- estados de motor precargados para ejercitar el sistema de notificaciones
-- durante el período de calibración pre-FASE 8 (calibration gate).
--
-- Uso:
--   SELECT seed_simulation_data('<uuid-del-owner>');
--
-- Limpieza:
--   DELETE FROM projects WHERE created_by = '<uuid>' AND icon = '🧪';
--
-- Diseño:
-- - Los triggers de engine (trg_obvs_phase, trg_obvs_probability, etc.) se
--   disparan al insertar OBVs y sobrescriben project_phase_state y
--   project_probability. Se compensa con DELETE del historial generado y
--   UPDATE explícito de todas las tablas de salida del motor DESPUÉS de
--   insertar OBVs. El estado simulado es siempre el estado final.
-- - run_notification_batch() se ejecuta al final con el estado simulado fijo.
-- - notifications.emailed_at no se toca (permite verificar canal email N7.6
--   de forma independiente).
-- - Las columnas de input de motores (phase_score_input, runway_factor_input,
--   etc.) quedan NULL — no son necesarias para las notificaciones.
-- ============================================================================
...
BEGIN
  ...
  -- Fallbacks (ids semilla de migración 00002)
  ...
  FOR v_spec IN SELECT jsonb_array_elements('[
    ...
    -- ── 1. Crear proyecto ──────────────────────────────────────────────────────
    -- El trigger fn_initialize_project_data crea automáticamente:
    -- project_phase_state, project_probability, project_viability_state,
    -- project_risk_score, project_function_coverage (con valores default Day 1).
    ...
    -- ── 3. Insertar OBVs ──────────────────────────────────────────────────────
    -- Nota: los triggers de engine (trg_obvs_phase, trg_obvs_probability)
    -- se disparan y ejecutan run_phase_engine / run_probability_engine.
    -- Los valores resultantes se SOBREESCRIBEN con los de simulación en los
    -- pasos 10-15. El historial generado se elimina en el paso 8.
    FOR i IN 0..(jsonb_array_length(v_spec->'obv_types') - 1) LOOP
      ...
      INSERT INTO obvs (
        ...
        v_obv_tipo::obv_type,
        v_evtype,
        -- ActivityWithoutOutcome: resultado 'partial' y kpi_status 'pending'
        -- (escenario edge case: actividad sin outcome definido).
        -- obv_outcome NOT NULL desde migración 00031.
    ...
    -- fecha_limite = hace i semanas → siempre < CURRENT_DATE → vencida.
    -- assignee_id = p_owner_id → notify_overdue_tasks() las detecta.
    ...
    IF (v_spec->>'has_strategic_block')::boolean THEN
      INSERT INTO strategic_blocks (
        ...
        -- created_by omitido: referencia auth.users(id), no profiles(id).
        -- Bloques con origin='engine' no requieren creador manual.
      ) VALUES (
        v_project_id,
    ...
    -- updated_at explícito en INSERT (trigger update_obvs_updated_at es
    -- BEFORE UPDATE, no BEFORE INSERT → el valor insertado se respeta).
    -- responsable_id = p_owner_id → notify_inactive_leads() lo detecta.
    -- obv_outcome NOT NULL desde migración 00031 → usar 'partial' (sin cierre).
    ...
    -- Los triggers de OBV generaron entradas con calculated_at ≈ NOW().
    -- Las borramos todas para insertar el historial simulado con timestamps
    -- correctos (delta_days_ago). Si no se hicieron entradas, DELETE es no-op.
    ...
    -- trigger_source: primera entrada → weekly_job; cambio de fase → acceleration
    -- o regression; misma fase → weekly_job. change_reason es nullable.
    v_prev_phase := NULL;
    ...
    -- SparseHistory tiene probability_history = [] → loop es no-op.
    ...
    -- Esto garantiza coherencia con G9.9: notify_phase_changes() lee el
    -- historial y compara v_current_phase (project_phase_state) con
    -- MAX(calculated_at) - 1. Para NotificationStress: fase=3 en JSON pero
    -- última entrada del historial es phase=2 → current_phase=2 → dispara
    -- phase_regressed correctamente.
    ...
    -- Constraint: probability_status='inactive' → probability_score MUST be NULL.
    v_prob_score_val := CASE (v_spec->>'probability_status')
      WHEN 'inactive' THEN NULL
      ELSE (v_spec->>'probability_score')::numeric
    END;
    UPDATE project_probability
    SET
      probability_score       = v_prob_score_val,
    ...
    -- Constraint: risk_status='insufficient_data' → risk_score MUST be NULL.
    v_risk_score_val := CASE (v_spec->>'risk_status')
    ...
    -- Aplica los mismos valores a las 3 funciones (demand/delivery/cash).
    -- Las columnas de score tienen CHECK IN (0, 30) o IN (0, 20) — solo
    -- valores discretos válidos del esquema se usan aquí.
  ...
  -- Con el estado simulado ya fijo en todas las tablas de output, el batch
  -- lee los valores correctos y genera las notificaciones esperadas.
