...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimitResult = await checkRateLimit(user.id, 'geo-intelligence', RateLimitPresets.AI_GENERATION);
...
Devuelve SOLO el JSON, sin explicaciones adicionales.`;
  ...
  await logAICall({
    ...
    inputData: { city, country, industry, businessType },
  ...
  if (!result.ok) throw new Error(`Failed to parse geo intelligence: ${result.error}`);
  return result.data;
}
